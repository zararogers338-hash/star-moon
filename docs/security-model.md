...
### Ordinary history is separate from encryption

“Ordinary” or “persistent” describes where ChatGPT keeps the conversation: the browser starts at
`https://chatgpt.com/`, and a submitted conversation may acquire a `/c/<id>` URL that remains in
the account's ChatGPT history. The runtime rejects `temporary-chat=true`, settings/auth pages,
backend routes, and lookalike origins as resumable conversation surfaces. This is a history and
session rule; it is not a claim that every payload is plaintext.

The browser-to-ChatGPT connection uses the normal HTTPS site. The local Responses endpoint is
loopback HTTP by design, and Full mode reaches the local Codex harness through OpenAI's outbound
HTTPS Secure MCP Tunnel. The `encrypted_function_args: []` field on the three collaboration calls
is Codex's explicit plaintext-delivery marker. The bridge-owned `ocx1:` and `ocxr1:` values are
base64-encoded transport envelopes, not cryptographic encryption; native OpenAI
`encrypted_content` values remain opaque and are never decoded or replaced.
...
authentication evidence: the launcher also requires a valid server session and the persistent
ChatGPT composer before setup can continue. No cookies, local storage, or browser profile are copied from an
external browser.

### Cross-turn data leakage

Browser turns use at most five independent task-bound tabs in one private login partition. Every
outer Codex task owns an exact launcher surface lease and retains its ordinary ChatGPT conversation only across
...
fresh tool-free persistent conversation receives the canonical Codex history; the bridge never parses ordinary
