...
Project Star & Moon keeps the local Responses/MCP path as its authoritative
execution route. The launcher offers a pinned download for the MIT-licensed
[Chat On Steroids companion](integrations/chat-on-steroids.md), which users may
load into a normal Chrome/Chromium profile. This is deliberately an optional
browser surface: installing an extension cannot grant filesystem, shell,
approval, or MCP permissions by itself.

The first-turn prompt compiler eagerly lists every MCP/app/plugin tool supplied
by the outer Codex request and asks the connected harness to enumerate the full
registry before task work begins. That keeps the ordinary persistent ChatGPT
conversation from losing deferred tools after the first message. The upstream
extension's richer Goal/Loop, session-journal, Compact & Resume, and worker-chat
protocol is versioned separately; Star & Moon does not claim those features until
its bridge adapter has passed the normal-browser reload/close/restart matrix.
...
- Sends the complete Codex context and image attachments to an ordinary persistent ChatGPT conversation.
...
then closes the old surface. The next epoch gets a new retained conversation surface. Model messages never copy
...
server-authenticated session and the persistent ChatGPT composer in the primary owned view, then closes
...
ordinary ChatGPT conversation and its locally compiled prompt is copied to the clipboard. A missing Automatic
retained source falls back to a dedicated read-only conversation built from canonical Codex
