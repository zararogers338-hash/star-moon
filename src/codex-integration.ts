...
function installConfiguredRoute(
  ...
  const hook = "interruptHookCommand" in config
    ? installCodexInterruptHookCommand(configured.text, getCodexConfigPath(), config.interruptHookCommand, replaceExistingRoute)
    : installCodexInterruptHook(configured.text, getCodexConfigPath(), config, replaceExistingRoute);
