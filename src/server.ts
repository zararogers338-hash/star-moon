...
import { cockpitChatCompletionsRequest, cockpitClientAuthorized, cockpitImagesEditRequest, cockpitImagesRequest, cockpitModelsRequest, forwardCockpitRequest, isCockpitModelSlug, cockpitUpstreamModel } from "./cockpit-provider";
...
import { readStoredCockpitProfile } from "./cockpit-profile";
...
import { forwardNativeCodexRequest, NativeCodexAuthorizationError, type NativeFetch } from "./native-passthrough";
import {
...
type HttpTrackedEndpoint = "models" | "responses" | "compact" | "images" | "search" | "unspecified";
...
function nativeProxyFailure(error: unknown): Response {
  if (error instanceof NativeCodexAuthorizationError) {
    const response = formatErrorResponse(401, "authentication_error", error.message);
    response.headers.set("www-authenticate", 'Bearer realm="Star Moon Codex bridge"');
    return response;
  }
...
export async function modelsRequest(
  ...
  if (cockpitClientAuthorized(req, config)) return cockpitModelsRequest(req, config, fetchUpstream);
  let upstream: Response;
  try {
    upstream = await forwardNativeCodexRequest(req, "models", fetchUpstream);
  } catch (error) {
    return nativeProxyFailure(error);
  }
  if (!upstream.ok) return upstream;
  let catalog: Record<string, unknown>;
  try {
    catalog = augmentNativeModelCatalog(await upstream.json(), config, contextOverride?.(), cockpitClientAuthorized(req, config));
  } catch (error) {
...
export async function responseRequest(
  ...
  if (typeof requestedModel === "string" && !isChatGptWebModelSlug(requestedModel)) {
    if (isCockpitModelSlug(requestedModel)) {
      if (!cockpitClientAuthorized(nativeRequest, config)) {
        return new Response("Unauthorized", { status: 401, headers: { "www-authenticate": 'Bearer realm="Star Moon Cockpit"' } });
      }
      try {
        return await forwardCockpitRequest(nativeRequest, "responses", config, { ...(raw as Record<string, unknown>), model: requestedModel });
      } catch (error) {
        return formatErrorResponse(502, "upstream_error", error instanceof Error ? error.message : String(error));
      }
    }
...
export async function compactRequest(
  ...
  if (isCockpitModelSlug(raw.model)) {
    if (!cockpitClientAuthorized(nativeRequest, config)) {
      return new Response("Unauthorized", { status: 401, headers: { "www-authenticate": 'Bearer realm="Star Moon Cockpit"' } });
    }
    try {
      // Keep this check explicit so an invalid prefix cannot be silently forwarded as a native model.
      cockpitUpstreamModel(raw.model);
      return await forwardCockpitRequest(nativeRequest, "responses/compact", config, raw);
    } catch (error) {
      return formatErrorResponse(502, "upstream_error", error instanceof Error ? error.message : String(error));
    }
  }
...
export function startServer(
  ...
  const currentConfig = (): AppConfig => {
    try {
      const stored = readStoredCockpitProfile();
      runtimeConfig = stored ? { ...config, cockpit: stored.cockpit } : config;
    } catch {
      // A malformed optional profile must not mutate or replace the original ChatGPT route.
      runtimeConfig = { ...config, cockpit: undefined };
    }
  ...
  let lastModelCatalogAttemptAt: string | null = null;
    ...
    async fetch(req) {
      ...
      if (req.method === "GET" && url.pathname === "/healthz") {
        return Response.json({
          ...
          last_model_catalog_attempt_at: lastModelCatalogAttemptAt,
          ...activity(),
        });
      }
      ...
      if (req.method === "GET" && url.pathname === "/v1/models") {
        ...
        return httpTurns.track(async signal => {
          ...
          try {
            ...
            catalogConfig = {
              ...requestConfig,
              subagentProtocol: readCodexSubagentProtocol(requestConfig.subagentProtocol),
            };
          } catch (error) {
            return formatErrorResponse(
              500,
          ...
          lastModelCatalogAttemptAt = new Date().toISOString();
          if (response.ok) {
      ...
      if (req.method === "POST" && url.pathname === "/v1/images/generations") {
        if (draining) return formatErrorResponse(503, "server_error", "codex-chatgpt-web is draining for a requested service operation");
        return httpTurns.track(
          signal => cockpitImagesRequest(new Request(req, { signal }), currentConfig(), dependencies.fetchUpstream),
          req.signal,
          process.platform,
          "images",
        );
      ...
      if (req.method === "POST" && url.pathname === "/v1/images/edits") {
        if (draining) return formatErrorResponse(503, "server_error", "codex-chatgpt-web is draining for a requested service operation");
        return httpTurns.track(
          signal => cockpitImagesEditRequest(new Request(req, { signal }), currentConfig(), dependencies.fetchUpstream),
          req.signal,
          process.platform,
          "images",
        );
      ...
      if (req.method === "POST" && url.pathname === "/v1/chat/completions") {
        if (draining) return formatErrorResponse(503, "server_error", "codex-chatgpt-web is draining for a requested service operation");
        return httpTurns.track(
          signal => cockpitChatCompletionsRequest(new Request(req, { signal }), currentConfig(), dependencies.fetchUpstream),
          req.signal,
          process.platform,
          "responses",
        );
