...
async function openChatGptStructuralPersonalizationState(
  ...
  try {
    ...
    if (!(error instanceof Error) || error.name !== "TimeoutError") throw error;
    throw chatGptConnectorUnavailableError(
      "ChatGPT persistent conversation did not expose a structural personalization control before the readiness deadline",
    );
  }
  const controlCount = await runChatGptPersonalizationStep(() => controls.count(), deadline, signal);
  if (controlCount !== 1) {
    throw chatGptConnectorUnavailableError(
      `ChatGPT persistent conversation exposed ${controlCount} structural personalization controls; expected exactly one`,
    );
  }
  await control.click({
    timeout: remainingChatGptPersonalizationMs(deadline, signal),
...
async function ensureChatGptPersonalizedConnectorAccessWithinDeadline(
  ...
  if (personalizedCount === 0 && unpersonalizedCount === 0) {
    ...
    if (personalizedCount === 0 && unpersonalizedCount === 0) {
      if (!proveConfiguredConnectorAccess) {
        ...
        throw chatGptConnectorUnavailableError(
          "ChatGPT persistent conversation did not expose a verifiable personalization control",
        );
      }
  ...
  if (personalizedCount !== 0 || unpersonalizedCount !== 1) {
    throw chatGptConnectorUnavailableError(
      `ChatGPT exposed an invalid persistent-conversation personalization state`
  ...
  try {
    ...
    if (!(error instanceof Error) || error.name !== "TimeoutError") throw error;
    throw chatGptConnectorUnavailableError(
      "ChatGPT did not confirm Personalized connector access for this persistent conversation",
    );
  }
  await capture("personalization-enabled");
  return "enabled";
...
/** ChatGPT may suppress connectors until this exact persistent conversation is Personalized. */
...
export async function dismissChatGptTemporaryChatOnboarding(page: Page): Promise<boolean> {
  ...
  if (!await continueButton.isVisible().catch(() => false)) {
    throw new Error("ChatGPT conversation onboarding is visible without its Continue action");
  }
...
export const browserStageTimeouts = {
  browserPage: 60_000,
  persistentChatPreparation: 150_000,
        ...
        withChatGptBrowserObservationTimeout(page.evaluate(({
          ...
          return {
            location: {
              ...
              temporaryChat: currentUrl.searchParams.get("temporary-chat")?.toLowerCase() === "true",
            },
...
export class ChatGptBrowserWorker {
  ...
  /**
   * A Codex turn owns one persistent ChatGPT document. Reusing the same
  ...
  /**
   * Put every browser operation on one fully hydrated persistent ChatGPT document.
   *
   * A new turn may start on the launcher's idle document and can therefore
   * navigate to ChatGPT.  A retained turn is different: navigating it would
   * discard the very transcript that makes continuation possible.  In that case
   * a missing/wrong surface is a terminal, explicit resume failure and the UI can
   * keep the user on the current setup page for recovery.
   */
  private async preparePersistentChatSurface(
    ...
    if (!isPersistentChatGptUrl(currentUrl)) {
    ...
    try {
      ...
      throw new Error("ChatGPT web login is expired or the persistent conversation surface is unavailable");
    }
    // Persistent conversations do not show the Temporary Chat onboarding dialog.
    await captureDiagnostic?.("composer-ready");
    await throwIfChatGptSessionFailureAlert(page);
    await assertAuthenticatedChatGptPage(page);
    await assertPersistentChatPage(page);
    await captureDiagnostic?.(options.allowNavigation === false
      ? "persistent-chat-resume-validated"
      : "session-verified");
    return composer;
  }
  ...
  private async selectConnector(
    ...
    let composer: Locator;
    // ChatGPT can render the same connector name in both the sidebar link and
    // the open composer menu.  The sidebar entry is an <a>, while the actionable
    // connector row is a div.  Restrict this locator to menu divs so duplicate
    // visible text cannot trigger Playwright strict-mode failures.
    const menuRows = page.locator('div.__menu-item[tabindex="0"]');
    const appResult = menuRows.filter({
      has: page.getByText(this.config.appName, { exact: true }),
    });
    // The old Temporary Chat route needed a Personalized/Unpersonalized
    // preflight.  That control is not part of an ordinary persistent ChatGPT
    // composer, so running it here causes every real persistent turn to wait
    // for a UI element that can never appear.  Keep the preflight for legacy
    // fixtures and non-ChatGPT pages, while persistent pages go straight to
    // the exact connector proof below.
    const pageUrl = typeof page.url === "function" ? page.url() : "";
    if (!isPersistentChatGptUrl(pageUrl)) {
  ...
  private async attachPrompt(
    ...
    try {
      // A retained browser tab is expected to keep the connector pill, but ChatGPT can clear a
      // composer selection after a reload, model switch, or account-side UI update.  Do not trust
      // the lease flag alone: prove the pill is still present and fall back to the exact connector
      // selection transaction when it is missing.  This makes AgentDock available even when the
      // later user prompt never names it again.
      if (connectorMode === "retained") {
        const retainedComposer = await this.activeComposer(page, 30_000, abortSignal);
        // The production worker always has connectorIsSelected. Keep the retained path tolerant of
        // older test/embedding shims that only provide the composer operations: those shims model a
        // connector-bound tab by construction, so re-selecting would be the harmful action.
        const connectorStillSelected = typeof this.connectorIsSelected === "function"
          ? await this.connectorIsSelected(retainedComposer, abortSignal)
          : true;
        if (!connectorStillSelected) {
  ...
  private async verifyConnectorExclusive(
    ...
    try {
      ...
      // Keep this entry point compatible with older launcher shims while the shipped worker uses
      // the persistent surface. A shim may provide the retired method for its own page fixture;
      // production always resolves the persistent method above it.
      const legacyWorker = this as unknown as {
        prepareTemporaryChatSurface?: (surface: Page, diagnostic?: (checkpoint: string) => Promise<void>) => Promise<void>;
      };
      const prepareSurface = typeof this.preparePersistentChatSurface === "function"
        ? this.preparePersistentChatSurface.bind(this)
        : typeof legacyWorker.prepareTemporaryChatSurface === "function"
          ? legacyWorker.prepareTemporaryChatSurface.bind(this)
          : undefined;
      if (!prepareSurface) throw new Error("ChatGPT browser surface preparation is unavailable");
      await prepareSurface(page, captureDiagnostic);
  ...
  private async inspectSessionExclusive(detectCapabilities: boolean): Promise<{
    ...
    const page = await this.ensurePage();
    await this.preparePersistentChatSurface(page);
    const url = page.url();
  ...
  private async runBrowserTurn(
    ...
    try {
      ...
      // Validate the surface on every turn.  A retained tab must stay on the
      // same ordinary ChatGPT conversation: if the user or a browser failure
      // moved it elsewhere, fail closed instead of navigating to the home page
      // and silently losing the continuation transcript.
      await this.runStage(
        turn.traceId,
        reuseConversation ? "persistent_chat_resume_validation" : "persistent_chat_preparation",
        browserStageTimeouts.persistentChatPreparation,
        () => this.preparePersistentChatSurface(
          page,
          checkpoint => diagnostics.capture(page, checkpoint),
          { allowNavigation: !reuseConversation },
        ),
      );
        ...
        try {
          ...
          await this.runStage(
            ...
            async () => {
              ...
              await this.preparePersistentChatSurface(
                page,
                checkpoint => diagnostics.capture(page, checkpoint),
              );
