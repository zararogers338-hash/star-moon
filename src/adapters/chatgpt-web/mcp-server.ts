...
import type { ExternalMcpClient, ExternalMcpTool } from "../../mcp-external";
...
export async function runChatGptMcpServer(options: {
  ...
  const externalMcpTools = options.externalMcp ? await options.externalMcp.listTools() : [];
  const externalToolsByWire = new Map<string, ExternalMcpTool>(externalMcpTools.map(tool => [tool.wireName, tool]));
  ...
  server.registerTool(
    ...
    async (input, extra) => withClaimedTurn(
      ...
      async claimed => {
        ...
        const externalMatches = externalMcpTools.filter(tool => !needle || `${tool.wireName}\n${tool.name}\n${tool.description}`.toLowerCase().includes(needle));
        let nestedTotal = 0;
        ...
        const externalOffset = Math.max(0, offset - directMatches.length - nestedTotal);
        const externalLimit = Math.max(0, limit - directPage.length - nestedPage.length);
        const externalPage = externalMatches.slice(externalOffset, externalOffset + externalLimit).map(tool => ({
          wire_name: tool.wireName,
          name: tool.name,
          namespace: "external_mcp",
          description: tool.description,
          kind: "external_mcp",
          ...(include_schema ? { parameters: tool.inputSchema } : {}),
        }));
        const page = [...directPage, ...nestedPage, ...externalPage];
        const total = directMatches.length + nestedTotal + externalMatches.length;
        return result({
          tools: page,
          total,
  ...
  server.registerTool(
    ...
    async (toolInput, extra) => {
      ...
      return withClaimedTurn("codex_tool_call", requestId, extra, async claimed => {
        const externalTool = externalToolsByWire.get(wire_name);
        if (externalTool) {
          if (!options.externalMcp) throw new Error("External MCP bridge is not available");
          if (input !== undefined) throw new Error(`External MCP tool ${wire_name} accepts structured arguments only`);
          // Normalize the external server's JSON-RPC result to the MCP SDK callback
          // shape. The external bridge deliberately exposes only JSON values, while
          // registerTool requires a response containing `content`.
          return result(await options.externalMcp.callTool(externalTool.name, args ?? {}));
        }
        const bound = claimed.environment;
