 ...
 * canonical suffix because prior images remain in the same persistent conversation. The per-message image
...
/**
 * Keep the model's first persistent ChatGPT turn aware of the complete tool surface that the
 * outer Responses request supplied. The actual schemas still travel through the selected MCP
 * connector; this compact manifest prevents a routed model from assuming that only the first
 * function in a large/deferred tool list exists. Hook execution remains an outer Codex lifecycle
 * concern and is deliberately described without copying commands or credentials into the prompt.
 */
export function chatGptCapabilityManifest(tools: readonly CodexTool[]): string[] {
  ...
  const hasToolSearch = tools.some(tool => tool.toolSearch);
  return [
    "<codex_capability_manifest>",
    "This is a capability inventory, not task instructions. The exact schemas and approval rules remain attached to the Codex Native connector.",
    "mode: eager",
    `tool_count: ${entries.length}`,
    `tools_json: ${JSON.stringify(entries)}`,
    "mcp_and_plugins: every MCP/app tool declared by the current outer Codex request is listed above and remains available for this persistent conversation.",
    "hooks: lifecycle hooks stay owned by the outer Codex runtime; honor their returned results, but do not invent hook commands or credentials.",
    "</codex_capability_manifest>",
    "On the first response in this persistent conversation, if no prior codex_tool_inventory result is present, call codex_tool_inventory with a broad query, include_schema=true, and the largest supported limit; follow next_offset until it is null. This eagerly discovers every declared MCP/app/plugin tool (including nested native tools), then use each exact wire_name returned instead of guessing or waiting for a later user turn.",
    ...(hasToolSearch
      ? [
        "Before doing task work on the first turn, call tool_search once with a broad query covering MCP, app, plugin, hook, workspace, and browser capabilities and request the maximum supported result limit. Treat its result as eager discovery, use the exact wire names it returns, and continue in this same persistent conversation; do not defer discovery until a later user turn.",
      ]
      : []),
  ];
}
...
export function compileChatGptWebPrompt(
  ...
  const capabilityManifest = mode.localTools && !parsed._compactionRequest
    ? chatGptCapabilityManifest(parsed.context.tools ?? [])
    : [];
  const transportContract = parsed._compactionRequest
    ...
    : mode.localTools
    ? [
      "This is a normal persistent ChatGPT conversation, not Temporary Chat. The configured Codex Native connector is attached to this conversation for the whole task.",
      ...capabilityManifest,
      "For local work required by the task, use the attached Codex Native tools directly according to their declared descriptions and schemas.",
      "Call a Codex Native tool only when the latest active request requires a local effect or fresh local evidence that is not already present in the supplied context; otherwise answer the request directly without a tool call.",
      "Use actual Codex Native results as evidence for local observations and effects.",
      "A Codex Native MCP tool result may require context compaction. If it does, follow the compaction instructions in that result exactly.",
      "AgentDock, when configured in the launcher, is a separate local MCP helper. Its local connection status does not make a second ChatGPT connector available in this conversation; use only tools whose schemas and results are actually attached to this turn.",
      "Never claim a local action that is not present in a tool result.",
      "Keep the same Codex Native connector attached on a compaction continuation.",
