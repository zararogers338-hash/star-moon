...
/**
 * The browser bridge deliberately uses an ordinary ChatGPT conversation.  A temporary chat
 * cannot be resumed reliably: it is omitted from history and its URL is not a stable conversation
 * identity.  Keep the URL at the origin so ChatGPT can create/restore the normal conversation in
 * the persistent browser profile.
 */
export const CHATGPT_CHAT_URL = "https://chatgpt.com/";
/** @deprecated Use CHATGPT_CHAT_URL. Kept as a source-compatible alias for downstream adapters. */
export const CHATGPT_TEMPORARY_CHAT_URL = CHATGPT_CHAT_URL;
...
/**
 * Return whether a ChatGPT URL is on a resumable ordinary-conversation surface.
 *
 * ChatGPT uses the same origin for the home page, conversations, settings, auth
 * pages, and backend routes.  An origin-only check therefore makes a settings or
 * login document look resumable and can cause a retained turn to attach to the
 * wrong document.  Keep this allowlist deliberately small: the launcher starts
 * at `/`, and a submitted ordinary turn is represented by `/c/<id>`.
 */
function isOrdinaryChatGptPath(url: URL): boolean {
  if (url.hash !== "") return false;
  if (url.pathname === "/") return true;
  return /^\/c\/[^/]+\/?$/.test(url.pathname);
}

export function isChatGptOriginUrl(value: string): boolean {
  try {
...
/**
 * Return whether a URL belongs to the ordinary ChatGPT conversation surface.
 *
 * This is deliberately a pure URL check.  It does not assert authentication or
 * a ready composer; callers still need to perform those checks separately.
 * Keeping the predicate shared prevents the launcher and Playwright helper from
 * accidentally disagreeing about what a resumable page is.
 */
export function isPersistentChatGptUrl(value: string): boolean {
  try {
    const url = new URL(value);
    const expected = new URL(CHATGPT_CHAT_URL);
    return url.origin === expected.origin
      && url.username === ""
      && url.password === ""
      && url.searchParams.get("temporary-chat")?.toLowerCase() !== "true"
      && isOrdinaryChatGptPath(url);
  } catch {
    return false;
  }
}

export async function assertPersistentChatPage(page: Page): Promise<void> {
  if (!isPersistentChatGptUrl(page.url())) {
    throw new Error(`ChatGPT is not on a persistent conversation surface (${page.url()})`);
  }
}

/** @deprecated Temporary chats are no longer an accepted runtime surface. */
export const assertTemporaryChatPage = assertPersistentChatPage;
