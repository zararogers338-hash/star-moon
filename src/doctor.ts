...
import { readClientRouting } from "./codex-client-status";
...
export async function runDoctor(): Promise<DoctorReport> {
  ...
  checks.push({ id: "codex-client", status: routing.compatible ? "ok" : "error",
    message: routing.compatible ? "User config is compatible with native catalog discovery; a real client request is still required"
      : "Saved Codex client settings bypass or block Star Moon; restarting alone will not resolve this",
    detail: JSON.stringify(routing) });
