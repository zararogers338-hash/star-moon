...
import { MANAGED_INTERRUPT_HOOK_START } from "./codex-interrupt-hook";
import type {
...
export function installRoute(
  ...
  if (currentBaseUrl.index !== undefined) {
    ...
    const firstTable = firstTableIndex(document.lines);
    const managedHook = document.lines.findIndex(line => line === MANAGED_INTERRUPT_HOOK_START);
    insertDocumentLine(document, managedHook >= 0 ? Math.min(firstTable, managedHook) : firstTable,
