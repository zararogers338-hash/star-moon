...
/**
 * Responses Lite clients have used both an input item and a top-level `additional_tools` field
 * while carrying deferred MCP/app tools. The validated schema intentionally keeps the raw body for
 * replay, so accept the top-level spelling here as well and feed it through the same normalizer.
 */
function rawAdditionalToolSpecs(body: unknown): unknown[] {
  if (!isObj(body)) return [];
  const value = body.additional_tools;
  if (Array.isArray(value)) return value;
  if (isObj(value) && Array.isArray(value.tools)) return value.tools;
  return [];
}
...
export function parseRequest(body: unknown): CodexParsedRequest {
  ...
  const loadedToolSpecs: unknown[] = rawAdditionalToolSpecs(body);
