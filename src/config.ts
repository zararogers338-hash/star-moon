...
import { validateExternalMcpConfig, type ExternalMcpConfig } from "./mcp-external";
...
export interface AppConfig {
  ...
  /** Optional explicit Cockpit reverse-proxy route. Disabled when absent or enabled=false. */
  cockpit?: CockpitConfig;
  /** Optional isolated external MCP bridge consumed only by the DEV/Full MCP stdio server. */
...
export function loadConfig(): AppConfig {
  ...
  return withStoredCockpitProfile(parseConfig(JSON.parse(stripUtf8Bom(readFileSync(path, "utf8"))), path));
}
...
export function loadConfigForSetup(): AppConfig {
  ...
  return withStoredCockpitProfile(parseConfig(raw, path));
}

function withStoredCockpitProfile(config: AppConfig): AppConfig {
  if (config.cockpit) return config;
  const stored = readStoredCockpitProfile();
  return stored ? { ...config, cockpit: stored.cockpit } : config;
}
...
function parseConfig(value: unknown, path: string): AppConfig {
  ...
  const cockpit = parsed.cockpit === undefined ? undefined : validateCockpitConfig(parsed.cockpit);
  const externalMcp = parsed.externalMcp === undefined ? undefined : validateExternalMcpConfig(parsed.externalMcp);
