...
REPOSITORY="${STAR_MOON_REPOSITORY:-}"
VERSION="${STAR_MOON_VERSION:-}"
OS="$(uname -s)"
MACHINE="$(uname -m)"

if ! printf '%s\n' "$REPOSITORY" | grep -Eq '^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$'; then
  echo "Set STAR_MOON_REPOSITORY=OWNER/REPO for the Star Moon release feed" >&2
  exit 1
fi
...
case "$OS" in
  ...
  Linux)
    ...
    ;;
  *) echo "Use install-star-moon.ps1 on Windows; unsupported OS: $OS" >&2; exit 1 ;;
esac
...
VERSION="${VERSION#v}"
if [ -z "$VERSION" ]; then
  echo "Could not resolve the latest Star Moon release" >&2
  exit 1
fi
case "$VERSION" in
...
ASSET="star-moon-$VERSION-$PLATFORM-$ARCH.$EXTENSION"
BASE_URL="https://github.com/$REPOSITORY/releases/download/v$VERSION"
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/star-moon-launcher.XXXXXX")"
...
if [ "$OS" = "Darwin" ]; then
  INSTALL_DIR="${STAR_MOON_APPLICATIONS_DIR:-/Applications}"
  STAGE_DIR="$TEMP_DIR/stage"
  mkdir "$STAGE_DIR"
  ditto -x -k "$TEMP_DIR/$ASSET" "$STAGE_DIR"
  SOURCE_APP="$STAGE_DIR/Star Moon.app"
  if [ ! -d "$SOURCE_APP" ] || [ ! -x "$SOURCE_APP/Contents/MacOS/Star Moon" ]; then
  ...
  TARGET_APP="$INSTALL_DIR/Star Moon.app"
  if pgrep -x "Star Moon" >/dev/null 2>&1; then
    echo "Quit Star Moon before updating it" >&2
    exit 1
  fi
  BACKUP_APP="$TEMP_DIR/Star Moon.previous.app"
...
LIB_DIR="${STAR_MOON_LIB_DIR:-$HOME/.local/lib/star-moon}"
BIN_DIR="${STAR_MOON_BIN_DIR:-$HOME/.local/bin}"
TARGET_DIR="$LIB_DIR/$VERSION"
TARGET="$TARGET_DIR/Star Moon.AppImage"
WRAPPER="$BIN_DIR/star-moon"
CORE_HOME="${STAR_MOON_HOME:-$HOME/.star-moon}"
...
if { [ -n "$RUNNING_PID" ] && kill -0 "$RUNNING_PID" 2>/dev/null; } \
  || pgrep -f "Star Moon\\.AppImage" >/dev/null 2>&1; then
  echo "Quit Star Moon before updating it" >&2
  exit 1
fi
...
RUNNER_QUOTED="$(shell_quote "$RUNNER")"
{
  printf '%s\n' '#!/bin/sh'
  printf '%s\n' 'set -eu'
  printf 'export STAR_MOON_LAUNCHER_EXECUTABLE=%s\n' "$WRAPPER_QUOTED"
  printf 'export STAR_MOON_APPIMAGE=%s\n' "$TARGET_QUOTED"
...
install -m 0644 "$ICON_SOURCE" "$ICON_DIR/star-moon.png"
DESKTOP_WRAPPER="$(printf '%s' "$WRAPPER" | sed \
  ...
  -e 's/\$/\\$/g' \
  -e 's/%/%%/g')"
cat > "$APPLICATIONS_DIR/star-moon.desktop" <<EOF
[Desktop Entry]
Type=Application
Version=1.0
Name=Star Moon
...
StartupWMClass=star-moon
EOF
chmod 0644 "$APPLICATIONS_DIR/star-moon.desktop"
