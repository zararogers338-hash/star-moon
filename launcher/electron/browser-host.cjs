...
const { loadLoginEntry, loginCancelledError, waitForLoginOperation } = require("./login-navigation.cjs");
const {
...
// All managed turns use an ordinary ChatGPT conversation.  Keep the old temporary URL as a
// compatibility export for older callers/tests, but never navigate to it from the runtime.
const CHATGPT_CHAT_URL = "https://chatgpt.com/";
const TEMPORARY_CHAT_URL = "https://chatgpt.com/?temporary-chat=true";
const CHATGPT_ORIGIN = "https://chatgpt.com";
const CONNECTOR_SETTINGS_URL = "https://chatgpt.com/#settings/Plugins";
...
function allowedLoginSurfaceUrl(value) {
  try {
...
function isChatGptOriginUrl(value) {
  try {
    const parsed = new URL(value);
    return parsed.origin === CHATGPT_ORIGIN && !parsed.username && !parsed.password;
  } catch {
    return false;
  }
}
...
function isPersistentChatUrl(value) {
  let parsed;
  try {
    parsed = new URL(value);
  } catch {
...
class BrowserHost {
  constructor({
    ...
    const expectedPartition = profile === "development"
      ? "persist:star-moon-dev-chatgpt"
      : "persist:star-moon-chatgpt";
    ...
    this.state = {
      ...
      canGoBack: false,
      canGoForward: false,
      zoomFactor: ZOOM_FACTORS.includes(initialZoomFactor) ? initialZoomFactor : 1,
    };
  ...
  bindWebContents() {
    ...
    contents.on("did-navigate", (_event, url) => {
      // Stop the navigation watchdog at main-document commit, not window.onload.
      // Stopping a streamed auth document after commit leaves its visible form
      // without the scripts that make Continue work.
      this.clearHomeNavigationTimeout();
      if (allowedAuthUrl(url)) this.logger.info("browser.auth_document_committed", {
        surface: "primary", origin: navigationOriginForLog(url),
      });
    });
    contents.on("did-finish-load", () => {
      this.clearHomeNavigationTimeout();
      ...
      if (this.manualOperation === "connector settings" || url === CONNECTOR_SETTINGS_URL) {
    ...
    contents.on("render-process-gone", (_event, details) => {
      this.clearHomeNavigationTimeout();
      const message = `Browser renderer stopped: ${details.reason}`;
      this.logger.error("browser.renderer_gone", { reason: details.reason, exitCode: details.exitCode });
      // Wake an in-flight login wait immediately. Otherwise waitForAuthenticated keeps polling
      // a destroyed WebContents until its full timeout and the UI appears permanently stuck.
  ...
  async hardRefreshHome(timeoutMs = BROWSER_NAVIGATION_TIMEOUT_MS) {
    ...
    await new Promise((resolve, reject) => {
      ...
      // Electron's `did-start-navigation` signature is
      // (event, url, isInPlace, isMainFrame, frameProcessId, frameRoutingId).
      // The previous implementation treated the first argument as a details
      // object. In a real BrowserWindow that argument is the Electron event,
      // so `isMainFrame` was always undefined and every hard refresh waited
      // until the 60-second timeout. Keep a small compatibility path for the
      // object-shaped fixture used by older embedders, but use the native
      // signature for the production path.
      const onStarted = (_event, url, isInPlace, isMainFrame) => {
        if (typeof _event?.isMainFrame === "boolean") {
          if (_event.isMainFrame && !_event.isSameDocument) mainNavigationStarted = true;
          return;
        }
        if (isMainFrame === true && isInPlace !== true && typeof url === "string") {
  ...
  async refreshChatGptHomeDocument() {
    // A navigation from the idle host already creates a fresh persistent ChatGPT document. Reload
    // an existing conversation so the helper observes one authoritative SPA bootstrap.
    if (isPersistentChatUrl(this.view.webContents.getURL())) {
  ...
  createAuthView(options = {}, requestedUrl = "") {
    ...
    contents.on("did-start-navigation", (_event, url, inPlace, mainFrame) => {
      if (!mainFrame) return;
      if (inPlace) { this.setState({ url }); return; }
    ...
    contents.on("did-navigate", () => clearNavigationTimeout());
    contents.on("did-start-loading", () => this.setState({ loading: true }));
    contents.on("did-stop-loading", () => {
      clearNavigationTimeout();
      this.setState({ loading: false });
    ...
    contents.on("did-finish-load", () => {
      ...
      if (browserInteractionModeFor(this) === "automatic"
        && this.manualOperation !== "connector settings"
        && this.view.webContents.getURL() !== CONNECTOR_SETTINGS_URL) void this.probeAuthentication();
    });
    contents.on("page-title-updated", (_event, title) => {
      if (browserInteractionModeFor(this) === "manual") return;
      this.setState({ title: typeof title === "string" && title.trim() ? title.trim() : "ChatGPT" });
  ...
  async openConnectorSettings() {
    const assertAvailable = () => {
      ...
      if (operation) throw new Error(`ChatGPT browser is already busy with ${operation}`);
      if (this.sessionRefreshOperation) throw new Error("ChatGPT browser is refreshing its saved session");
      if (this.activeTraceId) throw new Error("Finish or cancel active ChatGPT turns before opening connector settings");
    };
    assertAvailable();
    // User-driven settings navigation only: no DOM, session or auth inspection.
    return await this.withManualOperation("connector settings", async () => {
      this.show();
      await this.view.webContents.loadURL(CONNECTOR_SETTINGS_URL);
      return this.snapshot();
    }, () => {
      // ready() can yield while another operation starts. Recheck before the
      // manual-operation wrapper selects home, so a busy surface is not stolen.
      assertAvailable();
      if (!this.view?.webContents || this.view.webContents.isDestroyed()) {
        throw new Error("ChatGPT home browser is unavailable");
      }
    });
  }

  openLogin({ reload = false } = {}) {
    ...
    const operation = (async () => {
      ...
      if (sessionRefresh) {
        try {
          await waitForLoginOperation(() => sessionRefresh, signal);
        } catch {
      ...
      return await this.withManualOperation("ChatGPT login", async () => {
        ...
        const contents = reload ? this.activeView().webContents : this.view.webContents;
        const current = contents.getURL();
        if (reload || !allowedLoginSurfaceUrl(current)) {
          await loadLoginEntry(contents, reload && allowedLoginSurfaceUrl(current) ? current : CHATGPT_CHAT_URL, {
            allowedUrl: allowedLoginSurfaceUrl, signal,
          });
        }
        await waitForLoginOperation(() => this.probeAuthentication(), signal);
        const authenticated = await this.waitForAuthenticated(15 * 60_000, signal);
        if (signal.aborted) throw loginCancelledError();
    ...
    const tracked = operation.catch(error => {
      if (error?.code === "login_cancelled") return this.snapshot();
      throw error;
    }).finally(() => {
  ...
  async cancelLogin() {
    const operation = this.loginOperation;
    this.loginController?.abort();
    if (operation && this.loginController) await operation;
    return this.snapshot();
  }

  async reloadLogin() {
    requireAutomaticBrowserInspection(this, "ChatGPT sign-in page recovery");
    if (this.activeTraceId || (this.manualOperation && this.manualOperation !== "ChatGPT login")) {
      throw new Error("Finish the active browser operation before reloading sign-in");
    }
    await this.cancelLogin();
    // Reload the existing GET destination, never replay a submitted form body.
    // The existing partition is retained; no cookies or credentials are imported.
    return this.openLogin({ reload: true });
  }
    ...
    const operation = (async () => {
      ...
      return await this.withManualOperation("ChatGPT passkey login", async () => {
        ...
        this.setState({
          authenticated: false,
          status: "loading",
          message: "Waiting for sign-in in the dedicated external browser",
          loading: true,
        });
  ...
  async probeAuthentication() {
    ...
    const probe = (contents) => contents.executeJavaScript(`(async () => {
      ...
      const isOrdinaryConversationPath = (actualUrl) => actualUrl.hash === ""
        && (actualUrl.pathname === "/" || /^\\/c\\/[^/]+\\/?$/.test(actualUrl.pathname));
      const readSurface = () => {
        ...
        return {
          ...
          persistent: actualUrl.origin === expectedUrl.origin
            && actualUrl.username === ""
            && actualUrl.password === ""
            && actualUrl.searchParams.get("temporary-chat")?.toLowerCase() !== "true"
            && isOrdinaryConversationPath(actualUrl),
    ...
    if (!(result.composer && result.persistent && result.sessionAuthenticated)
      && this.authView
  ...
  async waitForAuthenticated(timeoutMs = 180_000, signal) {
    ...
    while (Date.now() < deadline) {
      ...
      const state = await waitForLoginOperation(() => this.probeAuthentication(), signal);
      if (state.authenticated) return state;
      await waitForLoginOperation(() => sleep(750), signal);
    }
  ...
  async runSessionInspection(detectCapabilities = false) {
    ...
    if (!inspected
      ...
      || inspected.temporary !== false
      || typeof inspected.url !== "string"
      || !isPersistentChatUrl(inspected.url)) {
  ...
  async withManualOperation(name, action, beforeActivate) {
    await this.ready();
    if (this.activeTraceId) {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      if (error?.code !== "login_cancelled") this.setState({ status: "error", message });
      throw error;
    } finally {
