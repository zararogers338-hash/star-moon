...
function readState(filePath) {
  try {
    ...
    if (state.language !== null && state.language !== "en" && state.language !== "zh-CN" && state.language !== "zh-Hant" && state.language !== "ja" && state.language !== "fr" && state.language !== "ru" && state.language !== "de") {
    ...
    if (state.browserVisible !== undefined && typeof state.browserVisible !== "boolean") delete state.browserVisible;
    if (state.browserZoomFactor !== undefined
      && (!Number.isFinite(state.browserZoomFactor) || state.browserZoomFactor < 0.5 || state.browserZoomFactor > 2)) {
      delete state.browserZoomFactor;
    }
    ...
    for (const key of [
      ...
      "catalogCheckDeferred",
    ]) {
      if (state[key] !== undefined && typeof state[key] !== "boolean") delete state[key];
    }
    if (state.guidedSetupResumeStage !== undefined && !SETUP_STAGES.includes(state.guidedSetupResumeStage)) delete state.guidedSetupResumeStage;
    return state;
  } catch {
