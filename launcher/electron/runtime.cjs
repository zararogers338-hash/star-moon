...
const { selectSetupPort } = require("./setup-port.cjs");
const { resolveSystemLoginBrowser } = require("./system-login-browser.cjs");
...
class RuntimeHost {
  selectSetupPort() { return selectSetupPort(); }
  ...
  passkeyChromeExecutable() {
    if (this.platform === "linux") return resolveSystemLoginBrowser({ usable: candidate => usableExecutable(candidate, this.platform) });
    if (this.platform !== "darwin") throw new Error("External browser sign-in is currently supported only on macOS and Linux");
  ...
  cancelPasskeyLogin() {
    ...
    if (this.active !== "passkey-login" || !child || child.exitCode !== null
      || child.signalCode !== null || !child.stdin?.writable) return false;
    if (this.passkeyContinuationRequested) throw new Error("Session handoff is already being verified; wait for verification to finish");
    // EOF makes the owned capture process close its disposable browser and clean
    // its profile in finally. Do not kill unrelated Chromium processes.
    child.stdin.end();
    return true;
  }
  ...
  async capturePasskeyLogin() {
    ...
    // Keep the storage-state file beside one persistent, Star Moon-owned
    // login profile. The browser-login helper derives its profile directory
    // from this path; placing it directly under passkey-login prevents the
    // profile from being nested inside a transfer directory that cleanup
    // would remove after every handoff.
    const storageStatePath = path.join(parent, "storage-state.json");
    ...
    try {
      await this.run("passkey-login", [
        ...
        message: "Sign in in the dedicated external browser, then return and authorize this session handoff",
        successMessage: "External browser session captured for private Launcher verification",
  ...
  async run(name, args, options = {}) {
    const publish = value => { if (!options.quiet) this.publishOperation?.(value); };
    ...
    this.active = name;
    publish({ name, status: "running", message: options.message || name });
    ...
    try {
      ...
      const result = await new Promise((resolve, reject) => {
        ...
        collect(child.stdout, stdout, (line) => {
          ...
          publish({ name, status: "running", message: redactText(line) });
      ...
      publish({ name, status: "completed", message: options.successMessage || "Completed" });
      return result;
    } catch (error) {
      const message = redactText(error instanceof Error ? error.message : String(error));
      this.logger.error("runtime.operation_failed", { name, message });
      publish({ name, status: "failed", message });
  ...
  async cockpitStatus() {
    const result = await this.run("cockpit-status", ["cockpit", "status"], {
      message: "Reading Cockpit gateway",
      timeoutMs: 15_000,
      captureOnly: true,
    });
    return JSON.parse(result.stdout);
  ...
  async cockpitConfigure({ baseUrl, apiKeyFile, models = [], disabled = false } = {}) {
    if (typeof baseUrl !== "string" || !baseUrl.trim()) throw new Error("Cockpit base URL is required");
    if (typeof apiKeyFile !== "string" || !path.isAbsolute(apiKeyFile)) throw new Error("Cockpit API key file must be absolute");
    if (!Array.isArray(models) || models.some(model => typeof model !== "string")) throw new Error("Cockpit models must be strings");
    const args = ["cockpit", "configure", "--base-url", baseUrl, "--api-key-file", apiKeyFile];
    if (models.length) args.push("--models", models.join(","));
    if (disabled) args.push("--disabled");
    const result = await this.run("cockpit-configure", args, { message: "Saving Cockpit gateway", timeoutMs: 15_000, captureOnly: true });
    return JSON.parse(result.stdout);
  }

  async cockpitProbe() {
    const result = await this.run("cockpit-probe", ["cockpit", "probe"], {
      message: "Probing Cockpit models",
      timeoutMs: 15_000,
      acceptedExitCodes: [0, 1],
      captureOnly: true,
    });
  ...
  async cockpitRollback() {
    const result = await this.run("cockpit-rollback", ["cockpit", "rollback"], {
      message: "Rolling back Cockpit gateway",
      timeoutMs: 15_000,
      captureOnly: true,
    });
    return JSON.parse(result.stdout);
  ...
  async cockpitEvidence({ limit = 100 } = {}) {
    if (!Number.isInteger(limit) || limit < 1 || limit > 500) throw new Error("Cockpit evidence limit is invalid");
    const result = await this.run("cockpit-evidence", ["cockpit", "evidence", "--limit", String(limit)], {
      message: "Reading Cockpit evidence",
      timeoutMs: 15_000,
      captureOnly: true,
    });
    return JSON.parse(result.stdout);
  ...
  async cockpitClientKey() {
    const result = await this.run("cockpit-client-key", ["cockpit", "client-key"], {
      message: "Preparing Cockpit client key",
      timeoutMs: 15_000,
      captureOnly: true,
    });
    const key = result.stdout.trim();
    if (!/^[A-Za-z0-9_-]{16,256}$/.test(key)) throw new Error("Cockpit client key output was invalid");
    return key;
  }
  ...
  async catalogStatus() {
    this.assertProductionProfile("Codex catalog diagnostics");
    const result = await this.run("catalog-check", ["route", "client-status"], {
      embedded: true, quiet: true, timeoutMs: 8_000,
    });
    const status = JSON.parse(result.stdout);
    if (status?.version !== 1 || !["blocked", "proxy-unavailable", "observed", "authentication-required", "request-failed", "waiting-client"].includes(status.state)
      || status.routing?.scope !== "user-config-on-disk" || typeof status.routing.compatible !== "boolean"
      || !Array.isArray(status.routing.issues) || !Number.isSafeInteger(status.requests) || status.requests < 0
      || typeof status.healthy !== "boolean"
      || (status.state === "observed" && (!status.healthy || !status.routing.compatible || status.requests < 1))) {
      throw new Error("Invalid catalog diagnostic evidence");
    }
    return status;
  }

  async copyCatalog(templatePath, targetConfig) {
    const result = await this.run("copy-catalog", ["copy-catalog", templatePath, targetConfig], {
      embedded: true, quiet: true, captureOnly: true, timeoutMs: 15000,
    });
    return JSON.parse(result.stdout);
  }
  ...
  async connectBridgeRoute() {
    ...
    try {
      let current;
      try {
        current = await this.bridgeStatus(name);
      } catch (error) {
        // A route-status error can mean that the user's Codex config changed
        // underneath us. The runtime may already be listening, but it is not
        // usable without a verified route. Stop the launcher-owned process so
        // health checks cannot report a false-green service while startup is
        // failing closed.
        let cleanupError;
  ...
  async runSetup(name, args, options) {
    ...
    try {
      if (this.launcherProfile === "production" && !previousRuntime.configured && !args.includes("--port")) {
        args = [...args, "--port", String(await this.selectSetupPort())];
      }
      if (this.launcherProfile === "production") {
