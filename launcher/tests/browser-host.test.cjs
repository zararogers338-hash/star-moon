...
test("legacy Temporary Chat detection stays isolated from the persistent surface", () => {
  ...
  assert.equal(isTemporaryChatUrl("https://user:secret@chatgpt.com/?temporary-chat=true"), false);
  assert.equal(isTemporaryChatUrl("not a url"), false);
});

test("persistent launcher URL proof accepts only the ordinary resumable conversation surface", () => {
  for (const url of [
    "https://chatgpt.com/",
    "https://chatgpt.com/?model=gpt-4o",
    "https://chatgpt.com/c/conversation-123",
    "https://chatgpt.com/c/conversation-123/",
  ]) assert.equal(isPersistentChatUrl(url), true, url);
  for (const url of [
    ...
    "not a url",
  ]) assert.equal(isPersistentChatUrl(url), false, url);
});

test("runtime navigation never uses the deprecated temporary-chat compatibility URL", () => {
  const source = fs.readFileSync(resolve(__dirname, "../electron/browser-host.cjs"), "utf8");
  assert.deepEqual(source.match(/(?:loadURL|goto)\([^)]*TEMPORARY_CHAT_URL/g) || [], []);
});
...
test("session inspection rejects authenticated evidence outside ordinary ChatGPT history", async () => {
  const fixture = Object.assign(Object.create(BrowserHost.prototype), {
    helper: {},
    descriptorPath: "/runtime/launcher-browser.json",
    getConnectorName: () => "Codex Native",
    ...
    runBrowserHelperOperation: async () => ({
      type: "result",
      value: { authenticated: true, temporary: false, url: "https://chatgpt.com/settings" },
    }),
  });
  await assert.rejects(
    BrowserHost.prototype.runSessionInspection.call(fixture, false),
    /invalid ChatGPT session evidence/,
  );
});
...
test("launcher authentication requires the persistent composer and complete server session", async () => {
  const fixture = {
...
test("OAuth completion is re-proved on the primary persistent surface before login succeeds", async () => {
...
test("a successful primary login redirect is accepted on the persistent surface", async () => {
  let currentUrl = "https://chatgpt.com/";
  const loadedUrls = [];
  const fixture = {
    activeTraceId: null,
    ...
    view: {
      webContents: {
        ...
        executeJavaScript: async () => ({
          composer: true,
          persistent: currentUrl === "https://chatgpt.com/",
          sessionAuthenticated: true,
          readyState: "complete",
          url: currentUrl,
        }),
...
test("hard refresh ignores an old loading stop before its own main-frame navigation", async () => {
  ...
  contents.reloadIgnoringCache = () => {
    ...
    queueMicrotask(() => {
      contents.emit("did-stop-loading");
      contents.emit("did-start-navigation", {}, "https://chatgpt.com/", false, false);
