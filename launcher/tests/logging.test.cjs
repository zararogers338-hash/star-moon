...
test("nested generic tokens, API credentials and local access keys are redacted in live and exported logs", () => {
  const secrets = { token: "SENSITIVE_A", api_key: "SENSITIVE_B", experimental_bearer_token: "SENSITIVE_C", nested: { refreshToken: "SENSITIVE_D", password: "SENSITIVE_E" }, line: "agt_codex_SYNTHETIC_DO_NOT_RECORD" };
  assert.doesNotMatch(JSON.stringify(sanitize(secrets)), /SENSITIVE|SYNTHETIC/);
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "star-moon-redaction-test-"));
  try {
    const filePath = path.join(root, "source.jsonl"), destinationPath = path.join(root, "export.jsonl");
    fs.writeFileSync(filePath, JSON.stringify({ at: "2026-09-06T00:00:00Z", level: "info", event: "test", detail: secrets }) + "\n");
    assert.equal(exportSanitizedLogs({ filePath, destinationPath }), 1);
    assert.doesNotMatch(fs.readFileSync(destinationPath, "utf8"), /SENSITIVE|SYNTHETIC/);
