...
test("launcher stops an unrouted runtime when route status is inconsistent", async () => {
  const fixture = bridgeFixture({ active: true });
  ...
  await assert.rejects(fixture.host.connectBridgeRoute(), /Codex bridge route is inconsistent/);
  assert.deepEqual(fixture.calls, ["route status", "runtime:stop"]);
});

test("catalog check is read-only, quiet, bounded and rejects false-positive evidence", async () => {
  const fixture = hostFor({ mode: "browser-only" });
  const evidence = { version: 1, state: "blocked", routing: { scope: "user-config-on-disk", compatible: false, issues: ["custom-provider"] }, healthy: true, requests: 0 };
  fixture.host.run = async (_name, args, options) => {
    assert.deepEqual(args, ["route", "client-status"]);
    assert.equal(options.quiet, true);
    assert.equal(options.timeoutMs, 8000);
    return { stdout: JSON.stringify(evidence) };
  };
  assert.deepEqual(await fixture.host.catalogStatus(), evidence);
  evidence.state = "observed";
  await assert.rejects(fixture.host.catalogStatus(), /Invalid catalog diagnostic evidence/);
});
...
test("failed first-time setup removes its route before restoring the unconfigured state", async () => {
  ...
  host.selectSetupPort = async () => 17842;
  try {
    await assert.rejects(
    ...
    assert.deepEqual(calls.map((args) => args.join(" ")), [
      "setup --browser-only --port 17842 --preflight-only",
      "setup --browser-only --port 17842",
    ]);
...
test("macOS passkey capture uses a persistent isolated launcher-controlled profile", async () => {
  ...
  try {
    ...
    const storagePath = invocation.args[invocation.args.indexOf("--storage-state") + 1];
    assert.equal(storagePath, path.join(root, "passkey-login", "storage-state.json"));
    assert.equal(fs.existsSync(path.dirname(storagePath)), true);
...
test("external sign-in rejects unimplemented platforms even if IPC is invoked directly", () => {
  const fixture = hostFor(null).host;
  fixture.platform = "win32";
  assert.throws(() => fixture.passkeyChromeExecutable(), /supported only on macOS and Linux/);
});
