...
### The local health page is `200`, but startup still fails

The local health endpoint only proves that a process answered on loopback; it does not prove that
Codex is routed to that process. During a route conflict, the launcher now stops its own runtime as
part of the failed startup path, so a healthy-looking daemon is not left behind while the launcher
reports an unusable configuration. A previously running process may take a few seconds to exit.

Use the launcher’s **Settings → Run doctor** and the exact startup error as the source of truth. Do
not “fix” this by repeatedly restarting the daemon or by manually editing `~/.codex/config.toml`.
Choose one route owner, then use **Repair Codex setup** (or **Remove Codex integration** when
switching away) so the journal can verify the resulting configuration. If repair still fails, export
the privacy-safe log and include the route error; never attach the Codex config, route journal,
cookies, or API keys to a public issue.
...
- Complete ordinary sign-in inside the launcher-owned flow and wait until a persistent ChatGPT composer
  is visible.
...
launcher version, account tier, sign-in provider, and whether the persistent ChatGPT composer ever
...
control, composer, send button, persistent conversation, personalization, or an operational viewport usually
mean that the ChatGPT UI did not expose a structure the bridge can safely prove.

1. Update to the latest release.
2. Confirm that a normal ChatGPT conversation can be opened in the launcher and that the account is not
