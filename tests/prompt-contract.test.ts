...
test("persistent local-tool prompts eagerly carry the complete MCP/plugin inventory", () => {
  ...
  expect(compiled.text).toContain("<codex_capability_manifest>");
  expect(compiled.text).toContain("mode: eager");
  expect(compiled.text).toContain("tool_count: 3");
  expect(compiled.text).toContain('"wire_name":"mcp__workspace__read_file"');
  expect(compiled.text).toContain('"wire_name":"tool_search"');
  expect(compiled.text).toContain("every MCP/app tool declared by the current outer Codex request");
  expect(compiled.text).toContain("lifecycle hooks stay owned by the outer Codex runtime");
  expect(compiled.text).toContain("call codex_tool_inventory with a broad query");
  expect(compiled.text).toContain("follow next_offset until it is null");
  expect(compiled.text).toContain("call tool_search once with a broad query");
  expect(compiled.text).toContain("continue in this same persistent conversation");
});
