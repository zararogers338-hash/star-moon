...
test("passkey login authenticates in normal Chrome before isolated offline pipe capture", async () => {
  ...
  try {
    ...
    expect(existsSync(join(root, "transfer", "login-profile"))).toBe(true);
  } finally {
...
test("Snap Chromium uses only Star Moon's dedicated profile folder", () => {
  expect(browserLoginProfileParent("/snap/bin/chromium", "/private/transfer/state.json", "linux", "/fixture/user"))
    .toBe("/fixture/user/snap/chromium/common/star-moon-login");
  expect(browserLoginProfileParent("/usr/bin/google-chrome", "/private/transfer/state.json", "linux", "/fixture/user"))
