...
import { installCodexInterruptHook, MANAGED_INTERRUPT_HOOK_END } from "../src/codex-interrupt-hook";
import {
...
describe("reversible native Codex route integration", () => {
  test("Star Moon retries, disconnects, reconnects and uninstalls over an intact legacy hook with no end marker", () => {
    const { codexHome } = fixture();
    const configPath = join(codexHome, "config.toml");
    const old = installCodexInterruptHook('model_provider = "user"\n', configPath, { runtimeCommand: ["/legacy/runtime"] });
    const original = old.text.replace(MANAGED_INTERRUPT_HOOK_END, "") + '\n[model_providers.user]\nname = "Keep me"\n';
    writeFileSync(configPath, original);
    const config = nativeConfig("browser-only");
    config.runtimeCommand = ["/star-moon/runtime"];
    ...
    expect(first).toContain('model_provider = "user"');
    expect(first).toContain("/star-moon/runtime");
    installCodexIntegration(config, { replaceExistingRoute: true });
