...
const { resolveLauncherProfile } = require("../launcher/electron/profile.cjs");
...
function descriptorFile(
  ...
  const launcherProfile = resolveLauncherProfile({
    argv: profile === "development" ? ["electron", ".", "--dev-profile"] : ["electron", "."],
    env: {}, homeDir: join(root, "home"), appData: join(root, "home", ".config"),
  });
  ...
  writeFileSync(path, `${JSON.stringify({
    ...
    partition: launcherProfile.browserPartition,
...
test("runtime accepts profile-resolved Star Moon partitions and rejects legacy or cross-profile storage", () => {
  for (const profile of ["production", "development"] as const) {
    const path = descriptorFile("http://127.0.0.1:39111", profile);
    expect(readLauncherBrowserHostDescriptor(path).partition).toBe(profile === "production"
      ? "persist:star-moon-chatgpt" : "persist:star-moon-dev-chatgpt");
    const value = JSON.parse(readFileSync(path, "utf8"));
    for (const partition of ["persist:codex-web-gpt-chatgpt", "persist:codex-web-gpt-dev-chatgpt",
      profile === "production" ? "persist:star-moon-dev-chatgpt" : "persist:star-moon-chatgpt"]) {
      writeFileSync(path, `${JSON.stringify({ ...value, partition })}\n`, { mode: 0o600 });
...
test("launcher session verification rejects authenticated evidence outside ordinary ChatGPT history", async () => {
  const server = createServer(async (request, response) => {
    for await (const _chunk of request) { /* drain request */ }
    expect(request.url).toBe("/v1/session/inspect");
    response.writeHead(200, { "content-type": "application/json" });
  ...
  try {
    ...
    if (!address || typeof address === "string") throw new Error("test server has no port");
    const path = descriptorFile(`http://127.0.0.1:${address.port}`);
    await expect(inspectLauncherBrowserHost(path)).rejects.toThrow(
      "Launcher ChatGPT session could not be verified: Launcher returned invalid ChatGPT session evidence",
    );
  } finally {
    await new Promise<void>(resolve => server.close(() => resolve()));
  }
...
test("launcher profile checks reject cross-profile browser ownership", async () => {
  ...
  expect(readLauncherBrowserHostDescriptor(path)).toMatchObject({
    profile: "development",
    partition: "persist:star-moon-dev-chatgpt",
  });
