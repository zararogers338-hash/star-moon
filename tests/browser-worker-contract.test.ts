...
import { isPersistentChatGptUrl, parseChatGptEffortSliderState } from "../src/chatgpt-session";
...
test("persistent ChatGPT URL proof accepts only resumable ordinary conversation surfaces", () => {
  for (const url of [
    "https://chatgpt.com/",
    ...
    expect(isPersistentChatGptUrl(url)).toBe(true);
  }
  for (const url of [
    ...
    expect(isPersistentChatGptUrl(url)).toBe(false);
  }
});
