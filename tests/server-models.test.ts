...
test("missing or malformed catalog authorization is 401, never an upstream 502 or a token lookup", async () => {
  for (const authorization of ["", "Basic PRIVATE", "Bearer", "Bearer PRIVATE extra"]) {
    let forwarded = false;
    const response = await modelsRequest(new Request("http://127.0.0.1/v1/models", {
      headers: authorization ? { authorization } : {},
    }), defaultConfig("browser-only"), async () => { forwarded = true; throw new Error("must not contact upstream"); });
