...
ブリッジは、コンパイル済みの現在の Codex タスクコンテキストを通常の永続 ChatGPT 会話へ送り、
...
ローカル Responses ブリッジは、選択されたモデルのタスクだけをタスクに紐付いた通常の ChatGPT 会話へルーティングします。
...
- **継続的なタスクセッションとネイティブコンパクション。** 連続するメッセージは、タスクに紐付いた
  1 つの永続会話を再利用します。コンテキスト境界に達すると、保持中のエージェントがチェックポイントを書き、
...
- **任意の通常ブラウザーコンパニオン。** ランチャーは MIT ライセンスの固定版
  [Chat On Steroids 統合ガイド](docs/integrations/chat-on-steroids.md)を提供します。通常の Chrome/Chromium に
  手動で読み込めますが、ローカルファイル、シェル、承認、MCP 権限を自動で付与することはありません。

ランチャーは、サインイン済み ChatGPT アカウントと通常の会話履歴を使用します。プロンプトは
OpenAI によって処理され、アカウント設定と OpenAI の利用規約が適用されます。このプロジェクトは非公式です。
適用される OpenAI の利用規約とワークスペースポリシーを守る責任は利用者にあります。

## クイックスタート

このソースパッケージには確認済みの公開 Star Moon リリースリポジトリがないため、既定の
ダウンロード・更新フィードは有効にしていません。[`HOW-TO-USE.md`](HOW-TO-USE.md) に従って
ソースからビルドするか、自分のリリースリポジトリを明示してチェックサム検証済みインストーラーを
使用してください。これにより、別の保存先とルート識別子を持つ上流 Codex Web GPT バイナリを
誤ってインストールすることを防ぎます。
...
ブラウザー、永続 ChatGPT 会話、プロンプトコンパイラー、Responses parser、コンパクションハンドラーを通して実行します。
...
Linux で AppImage をビルドする場合は、固定版 `libnotify` と所有済み AppImage ツールセットの準備も必要です。
[Ubuntu 向けパッケージング](docs/ubuntu-packaging.md)の手順を使用してください。最終 ABI が検証できない場合、
パッケージャーは AppImage の公開を拒否します。
